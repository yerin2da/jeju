...
export default function Rest() {
    ...
    const jsonDelete = async (id) => {//item.id 값이 id 매개변수로 들어갑니다.
        await fetch(`${url}/${id}`,{
            method: 'DELETE',// ID에 대한 DELETE 요청을 보냅니다.
        });
        setTdata(tdata.filter(item => item.id !== id));//삭제된 항목을 제외한 새로운 데이터를 tdata에 업데이트
