                                               ...
                                               })
{
    return (
        <div
            ...
            className={`relative w-full h-100 p-6 flex flex-col`}>

            <div className="absolute bottom-0 left-0 w-full h-[60%] bg-gray-200"></div>{/* 오버레이 */}

            {/* 포스터 이미지 */}
            <div className={`h-40 xs:h-60 sm:h-80 overflow-hidden relative z-2 border border-[#91530c]`}>
                <img
            ...
            <div className={`relative z-2 py-4 flex flex-col justify-between flex-1`}>
                <p className={`font-bold text-base multi-ellipsis2`}>{item.title}</p>
                <p className={`font-semibold text-sm text pt-2`}>{item.period}</p>
            </div>
